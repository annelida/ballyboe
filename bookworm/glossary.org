...
A word that stands in for noun, often to avoid the need to repeat the
same noun over and over.

** Subject pronouns

Used as the subject of a verb, which means that it's the person or
thing performing the action.

** Object pronouns

Used as grammatical objects of the verb. An object
pronoun receives the action instead of doing the action itself.

** Number

It is an arithmetical value are used in counting and making
calculation.

** Integer

Is a whole number without a fractional part that can be positive,
negative or zero.

** Float

The number with a fractional part.

** String

Is a sequence of characters enclosed in single quotes or double quotes
with the same result. In Python string are used for representing
textual data.
...
Object whose value is unchangeable once they are created.

** Containers
Some objects contain references to other objects. Examples of
containers are tuples, lists and dictionaries.

** List

Is a data structure in Python that is a mutable, or changeable,
ordered sequence of elements. Each element or value that is inside of
a list is called an item.

** Ordered sequence

Strings, list and tuples are ordered sequences. These data structures
guarantee the output order as they specified initially.

Sets and dictionaries are unordered collections of objects.

** EOF

An end-of file. You can prodyce that with 'Ctrl-D'.
...
Is a file that generally contains a short self-contained set of
instructions, i.e., lines of code, that perform a specific task.

** Module

Are simply files with the “. py” extension containing Python code that
can be imported inside another Python Modules Operations Program.

** Function

Is a "self contained" modules of code that accomplish a specific
task. Functions usually "take in" data, process it, and "return" a
result. Once a function is written, it can be used over and over and
over again.

** Data model

Is an abstract model that organizes elements of data and standardizes
how they relate to one another and to the properties of real-world
entities.
...
Is a free and open-source Unix-like operating system descended from
the Berkeley Software Distribution.

** Open source software

is software with source code that anyone can inspect, modify, and
enhance.

** Token

Tokens or lexical units are the smallest fractions in the python
programme. A token is a set of one or more characters having a meaning
together.
...
Is a reserved word in a computer language that has a specific
meaning. Python keywords form the vocabulary of the python
...
Are a data type and can hold any value type, such as strings, numbers,
and more.
Literals return a value for an object of the specified type.
...
Some objects contain reference to other objects. These are called
...
Is a template language that renders the pages of your application.

** Template processor

Is software designed to combine templates with data (defined by a data
model) to produce resulting documents or programs.

** Back end

The part of a computer system or application that is not directly
accessed by the user, typically responsible for storing and
manipulating data.

** Front end

Is the development of the graphical user interface of a website
through the use of HTML, CSS, and JavaScript so users can view and
...
Is a pattern that denotes a class of alternative strings to match.

Regular expressions have a syntax in which a few characters are
special constructs and the rest are ordinary.
...
#+begin_quote
Is a low-level programming language that provides a symbolic
representation of a computer's machine code instructions. It is
designed to be readable by humans and closely corresponds to the
machine language instructions specific to a computer's architecture.
#+end_quote

#+begin_quote
The conversion process is referred to as /assembly/, as in
/assembling/ the source code.
#+end_quote

#+begin_quote
The computational step when an assembler is processing a program is
...
#+begin_quote
Consists of five parts: memory, a processing unit, input, output, and
a control unit. The computer program is contained in the computer's
memory. The control of the order in which the instructions are carried
out is performed by the control unit.
#+end_quote

** Data structure

Is a way of organizing and storing data in a computer program so that
it can be accessed and manipulated efficiently.

** Hash

Is a mathematical function that transforms a string of characters or
data of arbitrary size into a fixed-size value or key, known as /a
hash value/, /hash code/, /hash digest/, or simply /hash/.
...
Is a mathematical concept that defines a set of possible values and a
set of operations that can be performed on those values.

** Class attributes

Are variables that are shared by all instances of a class. They are
defined at the class level and are accessible through the class itself
or any of its instances.

** Algorithm

Is a set of well-defined instructions used to solve a problem or
perform a specific task. It is a step-by-step plan that specifies the
actions to be taken, the order in which they should be executed, and
the conditions under which each step should be performed.
...
A diagram that shows a process, made up of boxes representing steps,
decision, inputs and outputs.

** Combination

Is a selection of elements from a set, where the order of selection
does not matter.

** Permutations

Is an arrangement of elements from a set, where the order of selection
matters.
...
Software Development Kit, is a collection of software development
tools, libraries, documentation, and sample code that developers use
to create applications for specific platforms or frameworks.

** API

Application Programming Interface is a set of rules and protocols that
allows different software applications to communicate with each
other. APIs define the methods and data formats that applications can
...
a complete platform for developers to build, run, and manage
applications without the complexity of maintaining the underlying
infrastructure. PaaS typically includes hardware, software, and
development tools hosted by a third-party provider, allowing users to
...
centers. The goal is to provide high availability and performance
("speed") by distributing the service spatially relative to end users.
